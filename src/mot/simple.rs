...
mod tests {
    ...
    fn test_match_objects() {
        ...
        // let bboxes_one: Vec<Vec<i32>> = vec![vec![236, -25, 386, 35], vec![237, -24, 387, 36]];
        // let bboxes_two: Vec<Vec<i32>> = vec![vec![321, -25, 471, 35], vec![322, -24, 472, 36]];
        // let bboxes_three: Vec<Vec<i32>> = vec![vec![151, -25, 301, 35], vec![152, -24, 302, 36]];
        ...
        for (bbox_one, bbox_two, bbox_three) in itertools::izip!(bboxes_one, bboxes_two, bboxes_three) {
            let blob_one = super::SimpleBlob::new(crate::utils::Rect::new(bbox_one[0], bbox_one[1], bbox_one[2]-bbox_one[0], bbox_one[3]-bbox_one[1]));
            let blob_two = super::SimpleBlob::new(crate::utils::Rect::new(bbox_two[0],bbox_two[1],bbox_two[2] -bbox_two[0],bbox_two[3]- bbox_two[1]));
            let blob_three = super::SimpleBlob::new(crate::utils::Rect::new(bbox_three[0],bbox_three[1],bbox_three[2] -bbox_three[0],bbox_three[3]- bbox_three[1]));

            // match mot.match_objects(vec![blob_one, blob_two, blob_three]) {
            match mot.match_objects(vec![blob_one]) {
                Ok(_) => {
                    // println!("{:?}", mot.objects.len());
                },
                Err(err) => {
                    println!("{:?}", err);
                }
