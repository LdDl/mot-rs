...
use pathfinding::{
    matrix::Matrix,
    prelude::kuhn_munkres_min
};
use crate::utils::{Rect, iou};
...
impl ByteTracker {
    ...
    /// );
    /// ```
    pub fn new(max_disappeared: usize, min_iou: f32, high_thresh: f32, low_thresh: f32, algorithm: MatchingAlgorithm) -> Self {
        ByteTracker {
            max_disappeared,
            min_iou,
            high_thresh,
    ...
    /// 
    pub fn match_objects(&mut self, detections: &mut Vec<SimpleBlob>, confidences: &[f32]) -> Result<(), TrackerError> {
        ...
        // Get active tracks
        let active_track_ids: Vec<Uuid> = self.objects.iter()
            .filter(|(_, track)| track.get_no_match_times() < self.max_disappeared)
            .map(|(id, _)| *id)
            .collect();
        let active_track_bboxes: Vec<(Uuid, Rect)> = active_track_ids.iter()
        ...
        let high_detection_indices: Vec<usize> = detections.iter()
            .enumerate()
        ...
        if !active_track_bboxes.is_empty() && !high_detection_indices.is_empty() {
            ...
            let iou_matrix = self.create_iou_matrix(&active_track_bboxes, &high_detection_indices, detections);
            // Perform matching
            let matches = self.perform_matching(&iou_matrix, &active_track_bboxes, &high_detection_indices);
            // Process matches
        ...
        let unmatched_track_ids: Vec<Uuid> = active_track_ids.into_iter()
            .filter(|id| !matched_tracks.contains(id))
            .collect();
        let unmatched_track_bboxes: Vec<(Uuid, Rect)> = unmatched_track_ids.iter()
            .map(|id| (*id, self.objects.get(id).unwrap().get_bbox()))
            .collect();
        let low_detection_indices: Vec<usize> = detections.iter()
            .enumerate()
            .filter(|(i, _)| confidences[*i] < self.high_thresh && confidences[*i] >= self.low_thresh)
            .map(|(i, _)| i)
            .collect();
        ...
        if !unmatched_track_bboxes.is_empty() && !low_detection_indices.is_empty() {
            // Create IoU matrix
            let iou_matrix = self.create_iou_matrix(&unmatched_track_bboxes, &low_detection_indices, detections);
            // Perform matching
            let matches = self.perform_matching(&iou_matrix, &unmatched_track_bboxes, &low_detection_indices);
            // Process matches
            self.process_matches(
                matches, 
        ...
        self.objects.retain(|_, track| track.get_no_match_times() < self.max_disappeared);
    ...
    fn create_iou_matrix(&self, track_bboxes: &[(Uuid, Rect)], detection_indices: &[usize], detections: &[SimpleBlob]) -> Vec<Vec<f32>> {
    ...
    fn perform_greedy_matching(&self, iou_matrix: &[Vec<f32>], track_bboxes: &[(Uuid, Rect)], detection_indices: &[usize]) -> Vec<(usize, usize)> {
    ...
    fn process_matches(&mut self, matches: Vec<(usize, usize)>, track_bboxes: &[(Uuid, Rect)], detection_indices: &[usize], iou_matrix: &[Vec<f32>], detections_array: &mut Vec<SimpleBlob>, matched_tracks: &mut HashSet<Uuid>, matched_detections: &mut HashSet<usize>) -> Result<(), TrackerError> {
...
mod tests {
    ...
    fn test_match_objects_spread() {
        ...
        for (i, iteration) in bboxes_iterations.iter().enumerate() {
            let mut blobs: Vec<SimpleBlob> = iteration.iter().map(|bbox| {
                SimpleBlob::new_with_dt(bbox.clone(), dt)
            }).collect();
